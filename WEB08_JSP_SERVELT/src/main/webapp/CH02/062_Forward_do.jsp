...
<%
	...
	//19세 이하는 되돌아감. 19세 이상이면 통과페이지로 이동 (20세 미만이므로 입장이 블가합니다. 라는 경고창 표시)
	//20세 미만일때 "입장이 불가합니다" 라는 alert 창 표시하고 되돌아갑니다.
	if(age < 20){
%>
		<script>
			alert("20세 미만이므로 입장이 블가합니다.");
			history.go(-1); //뒤로 버튼을 클릭한 효과
			//location.href='061_ForwardForm.jsp'
			
		</script>		
		<!-- html head body 등이 없다고 해서 다른 태그를 사용하지 못하는게 아닙니다. 
		그들은 양식일 뿐 html 태그는 어디서든  사용이 가능합니다. 자바스크립트 포함 -->
<%
	}else{
		// 063_ForwardResult.jsp로 이름과 나이를 갖고 갑니다.
