...
<body>
<%
	String id_str = session.getId(); //현재 세셩의 고유 아이디(고유 코드가 부여되어 구성) 얻음
	...
	session.setMaxInactiveInterval(3600); //세션의 수면(단위 : 초)
	int inactive = session.getMaxInactiveInterval()/60; //현재 수명을추출해서 분단위로 변환
	boolean b_new = session.isNew();//최초 생성 상태 여부
%>
[1] 세션 ID는 [<%=id_str %>]입니다. <br><br>
